...
Server.get('/getIP', function (req, res) {
  ...
  // res.status(200).send(requestIP);
  request(`https://ipinfo.io/${requestIP}/json`, function (err, body, _ipData) {
    const ipData = JSON.parse(_ipData);
    if (err) res.send(requestIP);
    else {
      request('https://ipinfo.io/json', function (err, body, _serverData) {
        const serverData = JSON.parse(_serverData);
...
Server.listen(1024, function () {
  console.log('Speedtest Server is up and running', 'http://localhost:1024');
});
